...
import { canUploadPhoto, canUpdateMedia, canDeleteMedia } from '../../utils/permissions';
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          ...
          {canUploadPhoto(user) && (
            <button
              ...
              className="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:from-blue-700 hover:to-purple-700 transition-all shadow-md hover:shadow-lg font-medium text-sm"
            >
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 text-sm mb-6">
              {debouncedSearch
                ...
                : 'Be the first to upload a family photo.'}
            </p>
            {canUploadPhoto(user) && !debouncedSearch && (
              <button
                onClick={() => setUploadOpen(true)}
                className="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg font-medium text-sm"
              >
                <Plus className="w-4 h-4 mr-2" />
                Upload Photo
              </button>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {items.map((item) => (
                <MediaCard
                  key={item.id}
                  item={item}
                  onClick={(i) => navigate(`/media/photos/${i.id}`)}
